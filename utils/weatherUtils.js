...
async function getAllowedFields(stationId, dataType) {
  ...
  const sensorFields = Object.entries(stationSensors)
    .filter(([sensor, enabled]) => enabled && SENSOR_FIELD_MAP[dataType][sensor])
    .flatMap(([sensor]) => {
...
async function hasSensor(stationId, sensor) {
  try {
    ...
    const fieldsToCheck = Array.isArray(sensorField) ? sensorField : [sensorField];
  ...
  return async (req, res, next) => {
    try {
      ...
      if (fields) {
        ...
        const invalidFields = requestedFields.filter(f => !req.allowedFields.includes(f));
